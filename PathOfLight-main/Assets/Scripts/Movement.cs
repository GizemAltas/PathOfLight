    ...
    void Update()
    {
         if (Input.GetMouseButtonDown(0))
      {
          ...
          if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit,float.PositiveInfinity))
          {
                ...
                else if (hit.collider.gameObject.CompareTag("But3"))
                {
                    player.destination = new Vector3(3.5f,52,-46 );        // kullanýcý butona týkladýgýnda butona basabilecek mesafeye gelmesini istedik.
                    player.speed = speed;
                    print("butona týkladýn");
                    Cd.isTouch = true;
                }
